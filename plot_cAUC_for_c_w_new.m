    ...
    for region = 1:15
        ...
        for fm = 1:3
            ...
            for am = 1:6
                ...
                if exist(fullfile(path,Animal,'ana\LSTM',strcat('LSTM_LC-HC_stage5_region',num2str(region),'_frame',num2str(frame(1)),'-',num2str(frame(end)),'_','fold10.mat')))
                    netLSTM = load(fullfile(path,Animal,'ana\LSTM',strcat('LSTM_LC-HC_stage5_region',num2str(region),'_frame',num2str(frame(1)),'-',num2str(frame(end)),'_','fold6.mat')));
                    ...
                    PF = load(fullfile(path,'ANA\Performance_LC-HC_stage5_3seg.mat'));
                    PF_ACC = PF.ACC;
                    PF_AUC = PF.AUC;
                    PF_ACC(PF_ACC<=0) = nan;
                    PF_AUC(PF_AUC<=0) = nan;
                    avgACC = permute(nanmean(PF_ACC(am,5,region,:,:,1),[5]),[1 2 4 3 5 6]);
                    avgAUC = permute(nanmean(PF_AUC(am,5,region,:,:,1),[5]),[1 2 4 3 5 6]);
                    ACC_c(am,5,region,:) =avgACC;
                    AUC_c(am,5,region,:) = avgAUC;
                end
            end
        end
