...
for r = 1:14
    ...
    subplot(2,14,14*floor((r-1)/14)+mod(r-1,14)+1)
    %subplot(2,7,r)
    hold on
    for i = 1:size(LF_L,1)
        ...
        patch([t fliplr(t)], [yMean-ySEM fliplr(yMean+ySEM)], color(2,:), 'EdgeColor','none', 'FaceAlpha',0.25);
        for each = 1:size(y,1)
            %plot(t,y(each,:),Color = color(2,:),LineWidth=0.5,LineStyle='-.')
        end
        plot(t,yMean,Color = color(2,:)/2,LineWidth=1);
        for stage = 1:5
            h0 = kstest(y(:,1));
            h = kstest(y(:,stage));
            if (h0 + h) == 0
            ...
            if p < 0.001
                %plot([stage-0.1 stage stage+0.1],[1.1 1.1 1.1],'*')
            elseif p < 0.01
                %plot([stage-0.05 stage+0.05],[1.1 1.1],'*')
            elseif p < 0.05
                %plot([stage],[1.1],'*')
            end
        ...
        if p < 0.001
            plot([3-0.1 3 3+0.1],[1 1 1],'*',color=[0 0 0])
        elseif p < 0.01
            plot([3-0.05 3+0.05],[1 1],'*',color=[0 0 0])
        elseif p < 0.05
            plot([3],[1],'*',color=[0 0 0])
            zzz = y;
        end
    ...
    subplot(2,14,14+14*floor((r-1)/14)+mod(r-1,14)+1)
    hold on
    for i = 1:size(LF_L,1)
        %plot(HF_L(i,:),Color=[0 0 1])
        %plot(HF_H(i,:),Color=[1 0 0])
        %plot(1:5,HF_L(i,:),'*','Color',color(2,:),'MarkerSize',5)
        %plot(1:5,HF_H(i,:),'*','Color',color(1,:),'MarkerSize',5)
        %plot(mean(HF_L),Color=color(2,:))
        %plot(mean(HF_H),Color=color(1,:))
        y = HF_base;%L
        N = size(y,1);
        yMean = mean(y);
        ySEM = std(y)/sqrt(N);
        CI95 = tinv([0.025 0.975], N-1);
        yCI95 = bsxfun(@times, ySEM, CI95(:));
        patch([t fliplr(t)], [yMean-ySEM fliplr(yMean+ySEM)], color(8,:), 'EdgeColor','none', 'FaceAlpha',0.25);
        for each = 1:size(y,1)
            %plot(t,y(each,:),Color = color(2,:),LineWidth=0.5,LineStyle='-.')
        end
        plot(t,yMean,Color = color(8,:)/2,LineWidth=1);
        %plot(t,mean(y),Color = color(2,:));
