...
Day = 20220607;
if ~exist(fullfile('D:\WF\data',AnimalID,'combined_dff'), 'dir')
   mkdir(fullfile('D:\WF\data',AnimalID,'combined_dff'));
end
...
for i = 2:size(to_do_list,2)
    ...
    data_temp = load(fullfile('D:\WF\data',AnimalID,'dff',to_do_list{i}));
    data_temp = data_temp.data_dff;
    data_temp = imresize(data_temp,0.2);
    data_dff = cat(3,data_dff,data_temp);
end
save(fullfile('D:\WF\data',AnimalID,'combined_dff',strcat(string(Day),'.mat')),'data_dff','-v7.3');